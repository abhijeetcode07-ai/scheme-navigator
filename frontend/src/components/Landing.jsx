...
function BubbleMenu({ copy }) {
  ...
  return <header className="site-nav" aria-label={copy.nav.home}>
    ...
    <nav id="main-menu" className={`bubble-menu ${open ? 'is-open' : ''}`}>{items.map(([label, href], index) => <a key={label} className={`bubble-link bubble-link-${index + 1}`} href={href} onClick={() => setOpen(false)}>{label}</a>)}</nav>
  </header>
}

function MoltenMetal() {
  return <div className="molten-metal" aria-hidden="true"><div className="molten-orb molten-orb-one" /><div className="molten-orb molten-orb-two" /><div className="molten-orb molten-orb-three" /><div className="grain" /></div>
}
...
export default function Landing({ onStart, language = 'English', onLanguageChange }) {
