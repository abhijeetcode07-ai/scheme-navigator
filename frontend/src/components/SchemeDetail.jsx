...
export default function SchemeDetail({ scheme, language = 'English', aiDetail, aiStatus, onBack, onChecklist }) {
  ...
  const stageContent = [
    <><p>{aiDetail?.why || scheme.plainEligibility || scheme.officialEligibility || fallback.why}</p>{scheme.ageEligibility && <p className="detail-note"><b>Age signal:</b> {scheme.ageEligibility}</p>}{scheme.incomeCeiling && <p className="detail-note"><b>Income signal:</b> {scheme.incomeCeiling}</p>}</>,
    <><p>{aiDetail?.support || scheme.benefits || fallback.support}</p>{scheme.premium && <p className="detail-note"><b>Cost or contribution:</b> {scheme.premium}</p>}</>,
    <><p>{scheme.applicationMode || fallback.before}</p><p className="detail-note"><b>Application window:</b> {scheme.applicationWindow || 'Check the official portal.'}</p><div className="detail-actions"><button className="detail-primary" type="button" onClick={onChecklist}>{copy.checklist} <span>↗</span></button>{isRealLink(applyLink) ? <a className="detail-secondary" href={applyLink} target="_blank" rel="noreferrer">{copy.official} ↗</a> : <span className="detail-unavailable">{copy.linkNote}</span>}</div></>,
